...
func TestParser0() {
	table := []struct {
		...
		{"Select * From user Where DATE_FORMAT(birthday,'%m-%d') >= datas and DATE_FORMAT(birthday,'%m-%d') <= '07-08'", true},
		...
		{"DELETE from t1  WHERE t1.id=25", true},
		{"DELETE t1,t2 from t1 LEFT JOIN t2 ON t1.id=t2.id WHERE t1.id=25", true},
		// from join
		...
		{"update travel t, xyz z  set x = 1 , y= 2 where t.id = z.id and   t.user_id = '666666' and t.id <= '1240'", false},
		...
		{`select db, query, query_time, process_time, wait_time, time, user, txn_start_ts, conn_id, backoff_time, request_count, total_keys, process_keys, index_ids, is_internal, digest, stats, cop_proc_avg, cop_proc_p90, cop_proc_max, cop_wait_avg, cop_wait_p90, cop_wait_max, mem_max from slow_query where time>date_sub(now(),interval 10 minute) and query not like '%slow_query%' order by db, query_time desc `, true},
		{"delete from travel_content  where travel_id >= 721 and truncate(avg(a.full_price),2) = 2", true},
	}
